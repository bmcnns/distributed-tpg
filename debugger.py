...
class Debugger:
    ...
    def plotTeam(_team: Team, teamPopulation, ax) -> None:
            ...
            for program in team.programs:
                ...
                if program.action.value in Parameters.ACTIONS:
                    ...
                    for childTeam in teamPopulation:
                        if program.action.value != team.id and childTeam.id == str(program.action.value) and str(
                                childTeam.id) not in visited:
                            actionId: str = str(uuid.uuid4())
                            print(str(childTeam.id), " ", program.action.value, " ",
                                  str(childTeam.id) == str(program.action.value))
        ...
        edge_colors = [G.nodes[node]['edgecolor'] if G.nodes[node]['node_type'] == 'team' else 'none' for node in
                       G.nodes]
        ...
        labels_dict = {node: '' if node_types[node] == 'program' or node_types[node] == 'action' else node for node in
                       G.nodes}
        ...
        for action_node in action_nodes:
            bbox_props = dict(boxstyle="round,pad=0.3", fc="white", ec="black", alpha=0.7,
                              lw=1)  # Adjust the boxstyle and other parameters as needed
            ax.text(pos[action_node][0], pos[action_node][1], G.nodes[action_node]['label'], ha='center', va='center',
                    color='black', size=6, bbox=bbox_props)
        ...
        for program_node in program_nodes:
            ax.text(pos[program_node][0], pos[program_node][1], G.nodes[program_node]['shape'], ha='center',
                    va='center', color='black', size=8)
